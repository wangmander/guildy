...
- Branch: v2-pivot
- Latest patch: **Guildy 2.1 Feature 3, TC comparison matrix at `<commit-hash>`**. New `job_compensation` table (one row per job, all comp fields nullable, per-user RLS; migration `supabase/migrations/20260606000001_job_compensation.sql`, apply manually). `lib/compMatrix/colSeed.ts` (COL multipliers, US national avg = 1.0) + `lib/compMatrix/normalize.ts` (pure formulas + strict `parseMoneyString` for tc pre-fill). `components/app/tc-matrix.tsx` mounts full-width below the rail+board wrapper in `app/app/page.tsx`, renders only when a job is in Full Loop (`interview_loop`/`final`) or Offer. Per-job inline comp form (base, signing, annual bonus %, equity, vesting, location, benefits notes) persisted via `upsertJobCompAction`; side-by-side normalized matrix (year-1, steady-state, COL-adjusted steady-state) with max-value bolding on total rows. Expanded by default, collapsible. Free, no paywall. tc pre-fill is display-only on empty forms (single clean number only). No fixed-bottom-overlap presentation (deferred to a visual-polish pass). No prep/Stripe/kanban changes.
