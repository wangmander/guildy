...
- Branch: v2-pivot
- Latest patch: **Guildy 2.1 command center rail, Features 1-2 + collapse/goal patch at `<commit-hash>`**. Left rail (`components/app/command-rail.tsx`, fed by `app/app/page.tsx`) with StatsPanel + Job Source Advisor + Today panel. F1 (`322cf50`) shipped the rail + advisor (`lib/jobSourceAdvisor/boardRatings.ts` seed + `aiFallback.ts` Haiku fallback). F2 (`cc0f7f9`) shipped the Today panel (prep-due / quick-prep-gap / source-nudge, capped 3). This patch: collapsed the advisor by default (chevron expand), replaced the apply-pace fallback item with a persistent Apply goal meter pinned at the Today panel bottom (`loggedThisWeek` count / 15 target, board-rating bar style), and added a role-tailored ballpark line via new `lib/jobSourceAdvisor/applyBenchmarks.ts` (seniority drives the per-offer range, field is a descriptor). Prep-detection query confirmed correct, unchanged. No kanban, stats-panel, or advisor-selection changes.
  - Prior: **PostHog funnel + engagement events**: `cross_subdomain_cookie: true` for `.guildy.ai` distinct_id continuity; `posthog-provider.tsx` identifies on Supabase `SIGNED_IN` / mount, resets on `SIGNED_OUT`; events `upgrade_modal_viewed`, `upgrade_modal_subscribe_clicked`, `kanban_job_created`, `prep_overlay_opened`, `quick_prep_generated`. No PII, no business logic changes.
...
- Date: 2026-05-31
- Project status: Guildy 2.1 command center, Features 1-2 shipped + collapse/goal patch. Next: Feature 3 (TC Comparison matrix).
